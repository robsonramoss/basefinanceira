...
// Mapeamento: palavra-chave (lowercase) → parte do nome da categoria
// ==========================================
const CATEGORY_KEYWORDS: { keywords: string[]; categoryHint: string }[] = [
  ...
  { keywords: ['posto', 'gasolina', 'combustivel', 'shell', 'petrobras', 'ipiranga', 'br'], categoryHint: 'combust' },
  { keywords: ['farmacia', 'drogaria', 'droga', 'ultrafarma', 'drogasil', 'pacheco', 'pague menos', 'remedios'], categoryHint: 'farm' },
  { keywords: ['uber', 'cabify', '99taxi', '99pop', 'taxi', 'ônibus', 'metro', 'passagem', 'transporte'], categoryHint: 'transport' },
...
/**
 * Sugere uma categoria baseada nas palavras-chave do memo/descrição.
 * Retorna o hint da categoria (parte do nome) ou null.
 */
function guessCategory(memo: string, categories: { id: number; descricao: string }[]): { id: number; confidence: number } | null {
  ...
  for (const { keywords, categoryHint } of CATEGORY_KEYWORDS) {
    const keywordMatch = keywords.some(kw => normalizedMemo.includes(kw));
    if (keywordMatch) {
      // Encontrar categoria que contenha o hint no nome
      const matchedCat = categories.find(c =>
        c.descricao.toLowerCase().includes(categoryHint)
      );
      if (matchedCat) {
        return { id: matchedCat.id, confidence: 75 };
      }
    }
  }
  return null;
...
export function OFXReconciliationModal({
  ...
  const [cardDueDay, setCardDueDay] = useState<number | null>(null);
  ...
  // Buscar dados do cartão (dia de vencimento)
  useEffect(() => {
    if (!cardId) return;
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data) setCardDueDay(data.dia_vencimento);
      });
  }, [cardId]);
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const addState: Record<string, { selected: boolean; categoryId: number; description: string; suggestedCategoryId?: number; suggestionConfidence?: number }> = {};
    results.forEach(r => {
      if (r.status === 'ofx_only' && r.ofxTransaction) {
        const suggestion = guessCategory(r.ofxTransaction.memo, categories);
        addState[r.id] = {
          selected: false,
      ...
      Object.keys(next).forEach(id => {
        if (next[id].suggestedCategoryId && next[id].suggestedCategoryId! > 0) {
          next[id] = {
            ...next[id],
            selected: true,
            categoryId: next[id].suggestedCategoryId!,
          };
        }
      });
      return next;
  ...
  // Quantos itens têm sugestão de categoria disponível
  ...
  const handleSaveSelected = async () => {
    ...
    try {
      ...
      // Calcular data_prevista baseada no mês da fatura + dia de vencimento do cartão
      const [invoiceYear, invoiceMonthNum] = invoiceMonth.split('-').map(Number);
      
      // data_prevista = dia de vencimento do mês da fatura
      // Ex: fatura 2026-03, vencimento dia 15 → data_prevista = 2026-03-15
      const dueDay = cardDueDay || 10; // fallback para dia 10 se não encontrar
      const dataPrevista = `${invoiceYear}-${String(invoiceMonthNum).padStart(2, '0')}-${String(dueDay).padStart(2, '0')}`;
        ...
        return (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-[var(--bg-card)] border border-[var(--border-medium)] rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
              ...
              <p className="text-sm text-[var(--text-secondary)] text-center mb-1">
                Esta ação irá excluir <span className="text-red-400 font-semibold">{pendingCount} lançamento(s)</span> que existem no sistema mas não foram encontrados no extrato OFX.
              </p>
              <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 my-4 space-y-1.5 max-h-40 overflow-y-auto custom-scrollbar">
                {reconciliation.filter(r => r.status === 'system_only' && !deletedItems.has(r.id) && r.systemTransaction).map(r => (
                  <div key={r.id} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-xs font-mono text-red-400 flex-shrink-0">{formatCurrency(r.systemTransaction!.valor)}</span>
                  </div>
                ))}
              </div>
              <p className="text-[11px] text-red-400/60 text-center mb-4">
...
interface ReconciliationRowProps {
  ...
  addState?: { selected: boolean; categoryId: number; description: string; suggestedCategoryId?: number; suggestionConfidence?: number };
  onToggleAdd: () => void;
